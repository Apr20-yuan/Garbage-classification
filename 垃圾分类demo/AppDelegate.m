...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    wikiPage * wiki = [[wikiPage alloc]init];
    UINavigationController * nav1 = [[UINavigationController alloc]initWithRootViewController:home];
    UINavigationController * nav2 = [[UINavigationController alloc]initWithRootViewController:wiki];
    home.title = @"首页";
    home.view.backgroundColor = [UIColor colorWithRed:204/255.0 green:221/255.0 blue:201.0/255.0 alpha:1.0];
    wiki.title = @"百科";
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            if (@available(iOS 10.0, *)) {
                _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"____demo"];
            } else {
